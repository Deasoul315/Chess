...
export const wss = new WebSocketServer({ port: WEB_SOCKET_PORT });
...
wss.on("connection", (ws: WebSocket) => {
  ...
  ws.on("message", (message) => {
    ...
    if (payload.type === "MESSAGE" && payload.userType === "SPECTATOR") {
      ...
      let connection = null;
      for (const [, conn] of connections) {
        const spectator = conn.spectators?.find((s) => s.userName === userName);
      ...
      if (guestConn?.socket && guestConn.socket.readyState === WebSocket.OPEN) {
        recipients.add(guestConn.socket);
      }
    ...
    if (type === "PLAY") {
      ...
      const success = connection.master.move(userName, fromX, fromY, toX, toY);
